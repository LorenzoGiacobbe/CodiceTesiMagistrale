...
if [ $DEL -eq 0 ]; then
	...
	iverilog -o $VVP -DXNOR_DELAY='#0' -DNAND_DELAY='#0' -DXOR_DELAY='#0' -DGATES $TB
	# echo "simulating $VVP ..."
	vvp $VVP > ./logs/vvp_log
elif [ $DEL -eq 1 ]; then 
	VVP="./verilog/tb/vvp/correlation_del.vvp"
	iverilog -o $VVP -DXNOR_DELAY='#0.1' -DNAND_DELAY='#0.2' -DXOR_DELAY='#0.5' -DGATES $TB
	# echo "simulating $VVP ..."
	vvp $VVP > ./logs/vvp_log_del
elif [ $DEL -eq 2 ]; then
	VVP="./verilog/tb/vvp/correlation_in_del.vvp"
	iverilog -o $VVP -DXNOR_DELAY='#0.1' -DNAND_DELAY='#0.2' -DXOR_DELAY='#0.5' -DGATES -DIN_DEL $TB
	# echo "simulating $VVP ..."
